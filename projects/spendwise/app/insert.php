...
try {
  if (!isset($_REQUEST["expense_name"]) || !isset($_REQUEST["amount"]) || !isset($_REQUEST["category_name"])) {
    ...
    $query = "INSERT INTO expense (added_on, expense_name, amount, details, category_id) VALUES (?, ?, ?, ?, ?)";

    if ($stmt = mysqli_prepare($link, $query)) {
      mysqli_stmt_bind_param($stmt, 'ssisi', $_REQUEST["date"], $_REQUEST["expense_name"], $_REQUEST["amount"], $_REQUEST["details"], $category_id);
      mysqli_stmt_execute($stmt);
